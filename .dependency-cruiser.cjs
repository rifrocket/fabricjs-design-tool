...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: "error",
      comment:
        "FUTURE_IMPLEMENTATION.md Stage 6: the canonical, renderer-neutral document model (packages/core/src/document/canonicalDocument.ts) and the asset abstraction (packages/core/src/assets/**) must be loadable/saveable/usable with zero renderer present — no fabric, no DOM. Scoped to canonicalDocument.ts specifically, NOT the whole document/ directory: packages/core/src/document/snapshot.ts already has a legitimate, pre-existing 'fabric' import (StaticCanvas, for renderSnapshotThumbnail's offscreen canvas) that Stage 5 does not remove — that file is the intentionally Fabric-coupled compatibility layer, a different module with a different contract than canonicalDocument.ts. Widen this rule's `from` to cover more of document/ only if a specific new file is confirmed to need the same renderer-free guarantee.",
