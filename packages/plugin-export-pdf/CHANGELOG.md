...
- 8822338: Widen `PdfExportOptions.pageSize` with `"match-canvas"` (derives the PDF page size directly from
  the canvas' own pixel dimensions and a new `dpi` option, default 96) and a literal
  `{ widthMm, heightMm }`, so output can be genuinely print-ready — the page _is_ the design's
