...
"Yes (panel-slot wrapper)" means the package's `install()` just registers an existing component into a panel slot (`registry.registerPanel()`) rather than adding engine-level behavior. These five packages need that wrapper because their UI (align/distribute, snapping toggle, dev-tools panels, the effects gallery/stack) has nothing to attach to without one — unlike `plugin-pan-zoom`, which is plain hooks/functions with no panel of its own, the deliberate holdout. None of the five `Yes (panel-slot wrapper)` packages are bundled into `<DesignEditor>`'s built-in presets (they'd create a circular dependency on `@rifrocket/fdt-react` — see `packages/react/src/preset/builtinPresets.ts`); add them via `plugins.add` instead. `plugin-effects-panel` is excluded for the mirror-image reason of the other four: it's `plugin-effects` (already bundled in `default`/`minimal`) that would gain the circular `@rifrocket/fdt-react` dependency if its own package grew a panel, so the panel lives in this separate sibling package instead, with no dependency on `plugin-effects` itself (only on `engine.registry.effects`, generic to whatever got registered). `plugin-pages` is a different kind of holdout again: it's not an `EditorPlugin` at all (no `install(engine)`), since it owns a whole collection of engines rather than extending a single one — its `plugins` option is *itself* a list of ordinary `EditorPlugin`s, installed on every page's engine via `engine.useAll()`.
