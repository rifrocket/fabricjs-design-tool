...
**Status: all 11 original stages COMPLETED, plus a Stage 12 follow-up addendum (also COMPLETED).** See the progress tracker below for per-chunk detail and [THREE_D_READINESS_REASSESSMENT.md](./THREE_D_READINESS_REASSESSMENT.md) (Stage 11, updated post-Stage 12) for the closing proven/not-proven status check before any 3D implementation begins. Stage 12 resolves the two `EditorContext`/`PluginRegistry` gaps Stage 11's own reassessment found and flagged as the actual blocker to a second renderer's plugins — see the "Stage 12" section near the end of this document. The "whole roadmap" completion criteria and "explicitly out of scope" sections below describe the original 11-stage architecture only; Stage 12 was a separate, smaller follow-up scoped after that reassessment, not part of the originally-approved architecture.
...
|---|---|---|---|---|---|
| 0.1 | COMPLETED | none | build/typecheck/test/apps:build/depcruise all green | On branch `version-5` (not `main`); `apps/npm-verify` doesn't exist on this branch, spot-check N/A here. build: 21/21; typecheck: 32/32; test: 42/42 tasks; apps:build: 22/22; depcruise: 78 modules/268 deps, 0 violations | 19c0e37 |
...
## Stage 12 — `EditorContext`/`PluginRegistry` generics (follow-up to Stage 11's reassessment)

**Scope:** `THREE_D_READINESS_REASSESSMENT.md` (Stage 11) named two genuine, previously-undocumented gaps as the actual blocker before a second (non-Fabric) `EditorContext` implementation could author plugins the way Fabric plugins are today: `PluginRegistry.objectTypes` hardcoded to `ObjectTypeRegistry<FabricObject>`, and `EditorContext.use()`/`useAll()` hardcoded to `EditorPlugin<CanvasEngine>`. Stage 12 is a small, separately-scoped follow-up (not part of the originally-approved 11-stage architecture) that resolves exactly these two gaps — no 3D capability interfaces, no 3D library, no camera/lighting/materials work, and no fix to `PluginRegistry.exporters`/`.importers` (still Fabric-`Canvas`-coupled, a separate residual gap). Planned and implemented under the same one-chunk-at-a-time, full-monorepo-validated discipline as Stages 0–11 (see chunks 12.1–12.6 in the progress tracker above for full detail).

**Design:** `PluginRegistry<TNode extends SceneNode = FabricObject>` threads `TNode` only into `objectTypes` (12.1) — the exact `ObjectTypeRegistry<TNode>` precedent from Chunk 1.2, one level up. `EditorContext<TNode>.registry` was widened to `PluginRegistry<TNode>` (12.2). `EditorContext<TNode>.use()`/`useAll()` were widened from the bare `EditorPlugin` default to `EditorPlugin<EditorContext<TNode>>` (12.3) — the riskiest chunk, isolated on purpose, since the "zero plugin changes" claim rested on a two-level-nested bivariant-method-checking argument that this exact codebase's own history (Chunk 3.1, Stage 5) had gotten wrong before. It was verified against a real `tsc` run, not assumed, and held on the first attempt with no fallback needed. Chunk 12.4 is the actual proof: `packages/core/src/testing/mockRendererApi.ts`'s test suite now has a `MockEditorContext` class that literally `implements EditorContext<MockNode>` (replacing Chunk 9.5's `FakeEditorContext` shape-matching workaround), installing a plugin written in the exact `context.registry.objectTypes.register(...)` style shipped Fabric plugins use.

**Result:** zero changes to any of the 17 shipped plugins or `apps/*` anywhere across the whole stage (`git status --short` confirmed empty on every plugin/app directory, chunk 12.5). `THREE_D_READINESS_REASSESSMENT.md` was updated (12.6) to move both findings to "Proven" and reword its "Recommendation" — the next real step before 3D implementation is now the 3D-specific capability interfaces (camera/lighting/materials/raycasting), not this prerequisite.
