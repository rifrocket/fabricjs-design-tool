...
- `PagesManager` — `addPage`/`duplicatePage`/`copyPage`/`deletePage`/`renamePage`/`reorderPages`, `setLocked`/`setVisible`, `moveObjectsBetweenPages`, `seedFromDocument`, `hydrate`, `refreshThumbnail`, `getSnapshotForPersistence`
